...
use harness::{MinioHarness, ReplicaTimingConfig};
...
async fn replica_bootstraps_against_local_minio() {
    let harness = MinioHarness::new().await;
    let replica = harness
        .spawn_replica(
            1,
            ServerAddr {
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[serial]
async fn gc_converges_meta_lengths_across_replicas() {
    let harness = MinioHarness::new().await;
...
#[tokio::test(flavor = "multi_thread", worker_threads = 4)]
#[serial]
async fn shutdown_preserves_final_mutation_and_removes_departed_membership_descriptor() {
    let harness = MinioHarness::new().await;
    let timing = ReplicaTimingConfig {
