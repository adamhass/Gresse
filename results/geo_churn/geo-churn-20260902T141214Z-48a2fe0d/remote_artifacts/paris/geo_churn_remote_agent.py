...
def durability_journal_files(base_path: Path) -> tuple[Path, ...]:
    """Return every file owned by the split local durability journal."""
    return (
        base_path,
        base_path.with_name(f"{base_path.name}.snapshot.json"),
        base_path.with_name(f"{base_path.name}.mutations.jsonl"),
        base_path.with_name(f".{base_path.name}.lock"),
    )


def retire_durability_journal(base_path: Path) -> None:
    """Remove an identity-bearing journal after permanent departure."""
    for journal_file in durability_journal_files(base_path):
        journal_file.unlink(missing_ok=True)


def completed_replica_pid(result_dir: Path) -> int | None:
    """Return the effective PID from a completed replica bootstrap metric."""
    for metrics_path in sorted(result_dir.glob("server_*.csv")):
        try:
            with metrics_path.open(newline="") as handle:
                for row in csv.DictReader(handle):
                    if (
                        row.get("source") == "server"
                        and row.get("event") == "replica_init"
                        and row.get("phase") == "completed"
                    ):
                        pid = int(row["replica_pid"])
                        if pid > 0:
                            return pid
        except (OSError, csv.Error, KeyError, TypeError, ValueError):
            # Metrics are read while the replica is still appending them.
            continue
    return None
...
class Agent:
    ...
    def wait_until_bootstrapped(self, replica: Replica) -> None:
        ...
        while time.monotonic() < deadline:
            ...
            effective_pid = completed_replica_pid(replica.remote_dir)
    ...
    def stop(self, replica: Replica, graceful: bool, retire_durability: bool = False) -> None:
        ...
        if retire_durability and self.durable_recovery:
            retire_durability_journal(self.durability_path(replica))
