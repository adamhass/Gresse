...
Options: --s3-region REGION (eu-north-1), --instance-type TYPE (t3.small),
--ssh-user USER (ec2-user), --topology PATH, --force-topology,
...
controller's public IP, creates any missing default VPCs, provisions all six
...
key_name=""; controller_cidr=""; instance_type="t3.small"; ssh_user="ec2-user"; project="gresse-geo-churn"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --ssh-user) ssh_user=$2; shift 2;; --topology) topology=$2; shift 2;; --project) project=$2; shift 2;;
...
for cmd in aws python3; do command -v "$cmd" >/dev/null 2>&1 || { echo "Missing required command: $cmd" >&2; exit 1; }; done
...
destroy_resources() {
  local regions=(eu-north-1 eu-central-1 us-east-1 us-west-2 ap-southeast-1 ap-northeast-1)
...
regions=(stockholm:eu-north-1 frankfurt:eu-central-1 virginia:us-east-1 oregon:us-west-2 singapore:ap-southeast-1 tokyo:ap-northeast-1)
...
for entry in "${regions[@]}"; do
  ...
  if [[ "$auto_provision" == true && "$region" != eu-north-1 ]]; then
    if ! aws ec2 describe-key-pairs --region "$region" --key-names "$key_name" >/dev/null 2>&1; then
      progress "[$name/$region] Importing the generated EC2 key pair"
      aws ec2 import-key-pair --region "$region" --key-name "$key_name" --public-key-material "fileb://${key_path}.pub" >/dev/null
    fi
  fi
...
progress "Authorizing replica-to-replica traffic across all six VMs"
...
[[ -n "$key_path" ]] && topology_args+=(--identity-file "$key_path")
